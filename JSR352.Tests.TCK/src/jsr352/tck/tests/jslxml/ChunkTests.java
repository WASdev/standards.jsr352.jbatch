...
import jsr352.tck.specialized.MyItemReadListenerImpl;
import jsr352.tck.specialized.MySkipProcessListener;
import jsr352.tck.specialized.MySkipReadListener;
import jsr352.tck.specialized.MySkipWriteListener;
...
public class ChunkTests {
    ...
    public void testChunkOnErrorListener() throws Exception {
    	...
    	try {
            ...
            URL jobXMLURL = this.getClass().getResource("/chunkListenerTest.xml");
    ...
    public void testChunkRestartItemCount10() throws Exception {
        ...
        String METHOD = "testChunkRestartInterval5commit20";
        try {
            Reporter.log("Create job parameters for execution #1:<p>");
            Properties jobParams = new Properties();
            Reporter.log("execution.number=1<p>");
            ...
            URL jobXMLURL = this.getClass().getResource("/chunkrestartCheckpt10.xml");
    ...
    public void testChunkRestartChunk5() throws Exception {
        ...
        try {
            ...
            Reporter.log("Got Job execution id: " + lastExecutionId + "<p>");
            {
                ...
                Reporter.log("execution.number=2<p>");
                Reporter.log("app.arraysize=30<p>");
                Reporter.log("app.writepoints=9,12,15,18,21,24,27,30<p>");
                jobParametersOverride.put("execution.number", "2");
                jobParametersOverride.put("app.arraysize", "30");
    ...
    public void testChunkDefaultItemCount() throws Exception {
        ...
        try {
            ...
            URL jobXMLURL = this.getClass().getResource("/chunksizeDEFAULTcommitIntervalDEFAULT.xml");
    ...
    /*
     * @testName: testChunkRestartCustomChunkDefaults
     * @assertion: job will finish successfully with COMPLETED. 
     *             5.2.1.1 - Reader, 5.2.1.1.1 - Reader Properties,
     *             5.2.1.2 - Processor
     *             5.2.1.3 - Writer, 5.2.1.3.1 - Writer Properties
     *             5.2.1 - Chunk, item-count default value
     *             5.2.1 - Chunk item checkpointing/restart
     * 
     * @test_Strategy: start a job with no item-count specified and a custom checkpoint algorithm specified..
     *                  Batch artifact enforces that the checkpointing occurs at the custom checkpoints.
     *                  
     */
    @Test
    @org.junit.Test
    public void testChunkRestartCustomChunkDefaults() throws Exception {
        String METHOD = "testChunkRestartCustomChunkDefaults";
        ...
        try {
            ...
            Reporter.log("Locate job XML file: chunkCustomCheckpointDEFAULTcommitInterval.xml<p>");
            URL jobXMLURL = this.getClass().getResource("/chunkCustomCheckpointDEFAULTcommitInterval.xml");


            Reporter.log("Invoke startJobAndWaitForResult for execution #1<p>");
            JobExecution execution1 = jobOp.startJobAndWaitForResult("chunkCustomCheckpointDEFAULTcommitInterval", jobParams);
            Reporter.log("execution #1 JobExecution getBatchStatus()=" + execution1.getBatchStatus() + "<p>");
            Reporter.log("execution #1 JobExecution getExitStatus()=" + execution1.getExitStatus() + "<p>");
            assertWithMessage("Testing execution #1", BatchStatus.COMPLETED, execution1.getBatchStatus());
            assertWithMessage("Testing execution #1", "buffer size = 10", execution1.getExitStatus());
     ...
     * 
     * @test_Strategy: start a job with item-count specified at a value greater than the read data set.  
    ...
    public void testChunkSkipRead() throws Exception {
        ...
        try {
            ...
            // assertWithMessage("Testing execution #1", "COMPLETED",
    ...
    public void testChunkSkipWrite() throws Exception {
        ...
        try {
            ...
            Properties jobParams = new Properties();
            Reporter.log("execution.number=1<p>");
            Reporter.log("processrecord.fail=1,3<p>");
            Reporter.log("app.arraysize=30<p>");
            jobParams.put("execution.number", "1");
     ...
     * 
     * @test_Strategy: start 3 seperate jobs with item-count specified at a value greater than the read data set.
    ...
    public void testChunkItemListeners() throws Exception {
        ...
        try {
            ...
            URL jobXMLURL = this.getClass().getResource("/testListeners.xml");
